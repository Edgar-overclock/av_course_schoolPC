...
class CarNavigationNode(Node):
    ...
    def __init__(self):
        ...
        self.goal_pose_publisher = self.create_publisher(PoseStamped,
        "/planning/mission_planning/goal", 10)
        
        # Subscriber for vehicle position
        self.odom_listener = self.create_subscription(Odometry,
        "/localization/kinematic_state", self.odom_callback, 10)
        
        # Service client for changing operation mode
        self.change_mode_srv = self.create_client(ChangeOperationMode,
    ...
    def setup_initial_pose(self):
        ...
        initial_pose.pose.pose.position.x = 3776.80
        initial_pose.pose.pose.position.y = 73725.2
        initial_pose.pose.pose.orientation.z = -0.519286
        initial_pose.pose.pose.orientation.w = 0.854601
        time.sleep(10)
    ...
    def setup_goals(self):
        ...
        self.goal_poses = [
        {'x': 3670.26, 'y': 73743.5, 'xx': 0.0, 'yy': 0.0, 'zz':-0.97, 'w': 0.25},
        {'x': 3773.3, 'y': 73699.9, 'xx': 0.0, 'yy': 0.0, 'zz':0.27, 'w': 0.96},
        {'x': 3812.68, 'y': 73767.4, 'xx': 0.0, 'yy': 0.0, 'zz':-0.97, 'w': 0.24}
        ]
        self.publish_goal()
    ...
    def odom_callback(self, msg: Odometry):
        ...
        distance_to_goal = (((current_pose.position.x -
        goal_pose['x']) ** 2 +
